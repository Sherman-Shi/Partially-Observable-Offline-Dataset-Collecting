...
def preprocess_minigrid_observation(obs):
    if isinstance(obs, tuple):
        # If 'obs' is a tuple, we need to find out how to correctly access the 'image' and 'direction'
        # This line assumes 'obs' is a tuple where the first element is the dictionary containing 'image'
        obs = obs[0]  # Adjust this based on actual structure
    
    # Now 'obs' should be a dictionary
    direction = obs['direction']
    image = obs['image'].flatten()  # Flatten the image to a 1D array
    return np.concatenate(([direction], image))
...
class DataCollector_MiniGrid:
    ...
    def collect_data(self):
        ...
        _returns, t, done = 0, 0, False
        s = self.env.reset()
        s = preprocess_minigrid_observation(s)
        while len(data['rewards']) < self.num_data_points:
            ...
            #qpos, qvel = self.env.sim.data.qpos.ravel().copy(), self.env.sim.data.qvel.ravel().copy()
            ns, rew, done, _, _ = self.env.step(a)
            ns = preprocess_minigrid_observation(ns)
            # try:
            #     ns, rew, done, _, _ = self.env.step(a)
            # except:
            #     print('Lost connection. Resetting environment.')
            ...
            if terminal or timeout:
                ...
                traj_data = self.get_reset_data()
        # More detailed debug before conversion
        # for k in data:
        #     all_shapes = [np.array(x).shape for x in data[k]]
        #     print(f"Shapes of elements in {k}: {set(all_shapes)}")
